...
import type { RequestFileInterface } from "./Request/RequestFileInterface.mjs";
...
class Request extends IncomingMessage
{
	...
	private body: Record<string, unknown>|string|Array<Record<string, unknown>> = "";
	...
	public setRawBody(value: string): void
	{
		...
		switch (this.getContentType())
		{
			case "application/json":
				...
				const PARSED_BODY: unknown = JSON.parse(this.rawBody);

				if (!TypeGuard.IsRecord(PARSED_BODY) && !TypeGuard.IsArray(PARSED_BODY, { itemGuard: TypeGuard.IsRecord.bind(TypeGuard) }))
				{
					throw new Error("Parsed JSON body in request resulted in non Record value.");
				}
	 ...
	 * getBody
	 */
	public getBody(): Record<string, unknown>|string|Array<Record<string, unknown>>
	{
		return this.body;
	}
